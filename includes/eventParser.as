...
public function gameOver(clear:Boolean = false):void { //leaves text on screen unless clear is set to true
	...
	if (silly && rand(5) == 0 && flags[kFLAGS.HARDCORE_MODE] == 0) textChoices = 5 + rand(4); //20% chance of humourous badend texts
...
public function errorPrint(details:* = null):void {
	...
	if (sTrace) rawOutputText(" (including the above stack trace copy&pasted into the details),");
...
public function goNext(timeAmt:Number, needNext:Boolean):Boolean { //the time argument is never actually used atm, everything is done with timeQ instead...
...
private function goNextWrapped(timeAmt:Number, needNext:Boolean):Boolean {
	...
	while (timeQ > 0) {
		...
		/* Inform all time aware classes that it's time for large events to trigger. Note that timeChangeLarge could be called multiple times in a single tick of the clock, so any updates should happen in timeChange and any timeChangeLarge events need to make sure they cannot repeat within the same hour
		...
		//IMP GANGBAAAAANGA
		temp = player.statusEffectv1(StatusEffects.BirthedImps) * 2; //the more imps you create, the more often you get gangraped
		if (temp > 7) temp = 7;
		...
		if (player.findPerk(PerkLib.Diapause) < 0) {
			...
			if (player.findPerk(PerkLib.MaraesGiftFertility) >= 0) //DOUBLE PREGGERS SPEED
				if (player.pregnancyAdvance()) needNext = true; //make sure pregnancy texts aren't hidden
			if (player.findPerk(PerkLib.MagicalFertility) >= 0) //DOUBLE PREGGERS SPEED
				if (player.pregnancyAdvance()) needNext = true; //make sure pregnancy texts aren't hidden
			if (flags[kFLAGS.EVENT_PARSER_ESCAPE] == 1) {
				flags[kFLAGS.EVENT_PARSER_ESCAPE] = 0;
				return true;
			...
			if (player.findPerk(PerkLib.BroodMother) >= 0) //DOUBLE PREGGERS SPEED
				if (player.pregnancyAdvance()) needNext = true; //make sure pregnancy texts aren't hidden
			if (flags[kFLAGS.EVENT_PARSER_ESCAPE] == 1) {
				flags[kFLAGS.EVENT_PARSER_ESCAPE] = 0;
				return true;
		...
		if (player.hasStatusEffect(StatusEffects.LootEgg)) {
			...
			var itype:ItemType = [
					...
					[consumables.L_BRNEG,consumables.L_PRPEG,consumables.L_BLUEG,consumables.L_PNKEG,consumables.L_WHTEG,consumables.L_BLKEG]]
						[player.statusEffectByType(StatusEffects.Eggs).value2 || 0]
						[player.statusEffectByType(StatusEffects.Eggs).value1 || 0]
						||consumables.BROWNEG;
			player.removeStatusEffect(StatusEffects.LootEgg);
	...
	telAdre.umasShop.updateBonusDuration(timeAmt); //hanging the Uma massage update here, I think it should work...
	...
	if (player.armorName == "lusty maiden's armor") {
		...
		if ((player.hasCock() && !player.hasSheath()) || player.balls > 0) { //grew cock or balls
		...
		else if (!player.hasVagina()) { //lost pussy
		...
		else if (player.biggestTitSize() < 4) { //tits gone or too small
	...
	if (player.totalCocks() != 0) {
		...
		while (counter >= 0) {
			if (player.cocks[counter].cockType == CockTypesEnum.DOG || player.cocks[counter].cockType == CockTypesEnum.FOX) { //update cock type as dog/fox depending on whether the player resembles one more then the other
				if (player.dogScore() >= player.foxScore())
					 player.cocks[counter].cockType = CockTypesEnum.DOG;
				else player.cocks[counter].cockType = CockTypesEnum.FOX;
			}
			counter--;
