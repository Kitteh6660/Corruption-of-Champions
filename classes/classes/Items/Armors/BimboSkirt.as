	...
	public class BimboSkirt extends ArmorWithPerk {
		...
		override public function useText():void { //Produces any text seen when equipping the armor normally
			...
			if (wornLower && wornLower) {
				...
				outputText(" You realize that you probably won't be able to seduce most of your foes in this ludicrous outfit. For a moment you consider taking your " + game.player.lowerGarment.longName + " off, but then decide against it.");
				return;
			}
			...
			if (!wornUpper) {
				if (game.player.biggestTitSize() >= 8) {
					...
					outputText("The halter top of your sluttish outfit snugly embraces your " + game.player.breastDescript(0) + ". The sensation of a soft fabric on your bare [nipples] makes you feel warm and sexy. ");
					if (game.player.isLactating()) {
						outputText("You feel how the top of your dress becomes wet, as droplets of milk leak from your [nipples]. ");
					}
					game.dynStats("lus", 5);
			...
			if (!wornLower) {
				if (game.player.buttRating < 8) {
					...
					outputText("You feel how the fine fabric of your sluttish skirt doesn't quite cover your [ass]");
					if (game.player.hipRating > 8) {
						outputText(", and the smooth skirt is stretched by your [hips]. ");
					}
					else outputText(". ");
				}
