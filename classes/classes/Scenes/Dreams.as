package classes.Scenes {
	...
	public class Dreams extends BaseContent {
		...
		public function dreamSelect():Boolean {
			...
			temp = player.humanScore(); //Add human numbers to choices
			while (temp > 0) {
			...
			//LUST CHANGES (Well adjusted cuts gain in half!)
			var dreamLust:Number = dreamtemp;
			if (player.findPerk(PerkLib.Lusty) >= 0) dreamLust += (dreamtemp / 3); //Lusty increases by 1/3rd
			if (player.findPerk(PerkLib.WellAdjusted) >= 0) dreamLust = (dreamLust / 2); //Well Adjusted cuts by half
			if (dreamLust > 0) dynStats("lus", dreamLust);
			//ACTUAL DREAM TEXTS
			if (dreamtemp > 5) {
				daydream = choices[rand(choices.length)]; //roll for dream!
				//normal fantasies...
				if (daydream == 0) {
					if (player.cor <= 33) { 
				...
				//canine
					...
					//Female
						...
						//normal dream
						else outputText("being in heat, your cunt sopping wet with moisture and desire, intense pheromones pouring off you to make all the males rigid and ready.  ");
						if (inBimbo) player.orgasm('Vaginal',false);
					}
				}
				//horse
				...
				//cat
					...
					//MALE
					else outputText("prowling through the forest as a cat.  One dream in particular has you encountering a female cat in heat, and filling her womb to the brim with cum as you rake her insides with the barbs on your cat-cock. You dream that you impregnate every female you come across, making sure you fight off any competing males...  ");
				}
				//demon
				else if (daydream == 5) outputText("being used and abused by demons of all varieties.  ");
				//minotaur cum
				else if (daydream == 6) {
				...
				//Scylla + dom + dong
				...
				else if (daydream == 17) { //by: The Dark Master (Predator/prey dream is always chosen if the PC is genderless, if they are gendered, 50/50 shot of predator/prey dream or the brood dream)
					//Predator and prey
					if (player.gender == 0 || rand(2) == 0) { //Dream about running from a sexy predator.  Emphasize that you're the prey, they are the predator.  You almost want to be caught, but you're also afraid of being caught.  When you finally are caught, you wake up
					...
					//Brood Den
					else { //Dream about living in a borrow and having lots and lots of kids.  Many furred, big eared bodies are running around and you feel the pregnant belly of your mate, or feel your own pregnant belly
						...
						else outputText("Things blur for a moment, and you find yourself laying on the bed with your hands wrapped around your heavily pregnant belly.  The product of your fertility and the virility of your great mate, and soon to be added to the great brood you've already brought into the world...\n");
					}
				}
				else if (daydream == 18) bimboDream();
				doNext(playerMenu);
				return true;
			}
			return false;
		...
		public function bimboDream() : void {
			...
			if (dreamType == 3) { //Tits
				if (intensity == 0)
					...
					if (player.hasVagina())
						 outputText("wandering through the forest, cradling your full and lactating breasts, searching for the right person to give your milk to.  You come across a cute human boy and invite him to your breast.  He eagerly jumps into your arms and starts to suckle from your ample bosom.  You then guide his erect cock into your " + player.vaginaDescript(0) + " and engulf him.  The two of you stand there in eternal bliss as he suckles on your chest and thrusts into your womanhood, until you wake from the dream.  ");
					else outputText("wandering the forest, carrying full and lactating breasts.  You spot a creature of the forest looking down at you, but you feel no fear, only contentedness as you invite the creature to suckle from your breasts.  It eagerly jumps out of its hiding place to lick and suck at your full breast, before soon running back off into the woods.  You continue your wonderings, and meet many more creatures.  Each one you meet comes to you and begs you for its daily milk.  It fills you with great satisfaction to feed them all, and you feel that they would do anything for you if you asked them to.  It is a state of eternal bliss, until you wake from the dream.  ");
				}
				else {
					...
					else outputText("walking into Whitney's barn.  Your [nipples] are engorged and ready to be taken care of.  You see a familiar machinery along the wall easily attach the harnesses and lift up into position, hearing the machinery activate automatically.\n\n");
		...
		private function bimboWetDreamOrgasm(dreamType: Number = 4):void {
			...
			switch (dreamType) {
			case 0: player.orgasm('Lips',false);
					break;
			case 1: player.orgasm('Anal',false);
					break;
			case 2: player.orgasm('Vaginal',false);
					break;
			case 3: player.orgasm('Tits',false);
					break;
			default: player.orgasm('VaginalAnal',false);
			}
