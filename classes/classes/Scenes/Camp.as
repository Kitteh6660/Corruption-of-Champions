package classes.Scenes {
	...
	public class Camp extends NPCAwareContent {
		...
		//Pass the doCamp function up to CoC. This way doCamp is private but the CoC class itself can call it
		public function Camp(campInitialize:Function) { campInitialize(doCamp); }
		...
		private function doCamp():void { //only called by playerMenu
			...
			if (ingnam.inIngnam) {
				getGame().ingnam.menuIngnam(); //Ingnam
				return;
			}
		...
		private function updateAchievements():void {
			...
			if (player.level >= 75) awardAchievement("Oh my god!", kACHIEVEMENTS.LEVEL_OH_MY_GOD);
			...
			if (NPCsBadEnds >= 7) awardAchievement("Because You Can", kACHIEVEMENTS.GENERAL_BAD_ENDER2);
			//Transformations
			if (player.hair.length == 0) awardAchievement("Bald Hero", kACHIEVEMENTS.GENERAL_BALD_HERO);
			...
			if (flags[kFLAGS.TIMES_MASTURBATED] >= 250) awardAchievement("Super-faper", kACHIEVEMENTS.GENERAL_SUPERFAP);
			if (flags[kFLAGS.TIMES_MASTURBATED] >= 500) awardAchievement("Fapobot", kACHIEVEMENTS.GENERAL_FAPOBOT);
			//Usual stuff
			...
			if (player.str >= 150 && player.tou >= 150 && player.spe >= 150 && player.inte >= 150) awardAchievement("Unimaginable Stats", kACHIEVEMENTS.GENERAL_STATS_150);
			if (player.str >= 200 && player.tou >= 200 && player.spe >= 200 && player.inte >= 200) awardAchievement("Supernatural Stats", kACHIEVEMENTS.GENERAL_STATS_200);
