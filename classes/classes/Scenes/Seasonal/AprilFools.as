package classes.Scenes.Seasonal {
	...
	public class AprilFools extends BaseContent {
		...
		public function isItAprilFools():Boolean { return (date.date == 1 && date.month == 3); }

		private function poniesFn():void {
				clearOutput();
				outputText(images.showImage("encounter-ponies"));
		...
		public function approachPonies():void {
			clearOutput();
			outputText(images.showImage("seasonal-ponies"));
		...
		public function derpCreepy():void {
			clearOutput();
			outputText(images.showImage("encounter-imp"));
		...
		public function derpyParty():void {
			clearOutput();
			outputText(images.showImage("seasonal-ponies-party"));
		...
		private function proceedToCheckout(method:String, nextFunc:Function = null):void {
			...
			else outputText("Fortunately, you can choose to pay with gems at a rate of $1 per 1000 gems! Proceed?");
			doYesNo(reallyCheckout, nextFunc);
		}
		//You just got TROLLED!
		private function reallyCheckout():void {
			clearOutput();
			outputText(images.showImage("event-trollface"));
		...
		public function pay2WinSelection():void {
			clearOutput();
			outputText(images.showImage("location-premshop"));
...
If you don't accept you never encounter them again the way i explain this one time event is that you accidently stumbled across this place, which is protected by magic. After that you never encounter them again*/