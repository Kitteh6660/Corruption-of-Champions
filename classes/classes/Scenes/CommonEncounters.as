...
package classes.Scenes {
	...
	public class CommonEncounters extends BaseContent implements Encounter {
		...
		public function encounterChance():Number { return theCommonEncounters.encounterChance(); }
		public function execEncounter():void { theCommonEncounters.execEncounter(); }
		public function encounterName():String { return "common"; }
		...
		//Helia sex ambush
		public function get theCommonEncounters():Encounter { return _tce ||= Encounters.group("common", kGAMECLASS.helScene.helSexualAmbushEncounter); }
		public function furriteMod():Number { return furriteFnGen()(); }
		public function lethiteMod():Number { return lethiteFnGen()(); }

		public function furriteFnGen(iftrue:Number = 3, iffalse:Number = 1):Function {
			return function ():Number { return player.findPerk(PerkLib.PiercedFurrite) >= 0 ? iftrue: iffalse; }
		}
		public function lethiteFnGen(iftrue:Number = 3, iffalse:Number = 1):Function {
			return function ():Number { return player.findPerk(PerkLib.PiercedLethite) >= 0 ? iftrue: iffalse; }
		}
		...
		private function twoImpsClashing():void {
			...
			unlockCodexImps();
			inventory.takeItem(randomChoice(consumables.SUCMILK, consumables.INCUBID, consumables.IMPFOOD), camp.returnToCampUseOneHour);
		}
		...
		/**@TODO merge*/
		//[FOREST/LAKE] RANDOM SCENE IF CHARACTER HAS AT LEAST ONE COCK LARGER THAN THEIR HEIGHT, AND THE TOTAL COMBINED WIDTH OF ALL THEIR COCKS IS TWELVE INCHES OR GREATER
		public function bigJunkForestScene(lake:Boolean = false):void {
			clearOutput();
			var x:Number = player.longestCock();
			outputText("Walking along the "); //PARAGRAPH 1
			...
			if (!player.isTaur()) {
				...
				if (player.biggestTitSize() >= 35) { //IF CHARACTER HAS GIANT BREASTS ADD SENTENCE
				...
				if (player.balls > 0) { //IF CHARACTER HAS A BALLS ADD SENTENCE
				...
				if (player.vaginas.length >= 1) { //IF CHARACTER HAS A VAGINA ADD SENTENCE
					...
					if (player.vaginas[0].vaginalWetness >= VaginaClass.WETNESS_DROOLING) { //IF CHARACTER HAS A DROOLING PUSSY ADD SENTENCE
			...
			if (lake) {
				if (player.canFly()) //SCENE END = IF CHARACTER HAS FULL WINGS ADD SENTENCE
					outputText("  You extend your wings and flap as hard as you can until at last, you manage to lighten the bulk of your body.  It helps just enough to let you drag your genitals out of the mud and back to camp.  The ordeal takes nearly an hour for you to return and deal with.");
				else if (player.isTaur()) //Taurs
					outputText("  You struggle and work your multiple legs against the wet ground.  Your " + player.feet() + " have consistent trouble finding footing as the mud fails to provide enough leverage to lift your bulk.  You breath in deeply and lean side to side, trying to find some easier vertical leverage beneath your feet.  Eventually, with a crude crawl, your legs manages to push the bulk of your body onto more solid ground.  With great difficulty, you spend the next hour shuffling your genitals back to camp.");
				else //SCENE END = FOR ALL OTHER CHARACTERS
					outputText("  You struggle and push with your " + player.legs() + " as hard as you can, but it's no use.  You do the only thing you can and begin stroking your " + player.multiCockDescriptLight() + " with as much vigor as you can muster.  Eventually, your body tenses and a light load of jizz erupts from your body, but the orgasm is truly mild compared to what you need.  You're far too weary from struggling to give yourself the masturbation you truly need, but you continue to try.  Nearly an hour later, " + player.sMultiCockDesc() + " has softened enough to allow you to stand again, and you make your way back to camp, still dragging your genitals through the mud.");
			}
			else {
				if (player.canFly()) //SCENE END = IF CHARACTER HAS FULL WINGS ADD SENTENCE
					outputText("  You extend your wings and flap as hard as you can, until at last, you manage to lighten the bulk of your body.  It helps just enough to let you drag your genitals out of the forest and back to camp.  The ordeal takes nearly an hour for you to return and deal with.");
				else if (player.isTaur()) //SCENE END IF CHARACTER HAS CENTAUR BODY
					outputText("  You struggle and work your multiple legs against the soft dirt.  Your " + player.feet() + " have consistent trouble finding footing as the ground fails to provide enough leverage to lift your bulk.  You breath in deeply and lean side to side, until eventually, your feet brace against the various roots of the trees around you.  With a crude crawl, your legs manage to shuffle your body and genitals out of the forest and back to camp.");
				else //SCENE END = FOR ALL OTHER CHARACTERS
		...
		//[DESERT] RANDOM SCENE IF CHARACTER HAS AT LEAST ONE COCK LARGER THAN THEIR HEIGHT, AND THE TOTAL COMBINED WIDTH OF ALL THEIR COCKS IS TWELVE INCHES OR GREATER
		public function bigJunkDesertScene():void {
			clearOutput();
			var x:Number = player.longestCock();
			outputText("Walking along the "); //PARAGRAPH 1
			if (rand(3) == 0) outputText("boundless sands");
			else outputText("sandy dunes");
			outputText(" of the desert you find yourself increasingly impeded by the bulk of your " + player.cockDescript(x) + " dragging along the sandscape behind you.  The incredibly hot surface of the desert causes your loins to sweat heavily and fills them with relentless heat.");
			...
			if (!player.isTaur()) {
				...
				if (player.balls > 0) //IF CHARACTER HAS A BALLS ADD SENTENCE
					outputText("  Your " + player.skin.tone + player.sackDescript() + " rests beneath your raised " + player.buttDescript() + ".  The fiery warmth of the desert caresses it, causing your " + player.ballsDescriptLight() + " to pulse with the need to release their sperm through your " + player.multiCockDescriptLight() + ".");
				if (player.vaginas.length >= 1) { //IF CHARACTER HAS A VAGINA ADD SENTENCE
					outputText("  Your " + player.vaginaDescript() + " and " + player.clitDescript() + " are thoroughly squashed between the bulky flesh where your male genitals protrude from between your hips and the " + player.buttDescript() + " above.");
					if (player.vaginas[0].vaginalWetness >= VaginaClass.WETNESS_DROOLING) //IF CHARACTER HAS A DROOLING PUSSY ADD SENTENCE
						outputText("  Juices stream from your womanhood and begin pooling on the hot sand beneath you.  Wisps of steam rise up into the air only to tease your genitals further.  ");
				}
			}
			//FOR CENTAURS
			else {
				...
				if (player.balls > 0) //IF CHARACTER HAS A BALLS ADD SENTENCE
					outputText("  Your " + player.skin.tone + player.sackDescript() + " rests beneath your raised " + player.buttDescript() + ".  The airy warmth of the desert teases it, causing your " + player.ballsDescriptLight() + " pulse with the need to release their sperm through your " + player.multiCockDescriptLight() + ".");
				if (player.vaginas.length >= 1) { //IF CHARACTER HAS A VAGINA ADD SENTENCE
					outputText("  Your " + player.vaginaDescript() + " and " + player.clitDescript() + " are thoroughly squashed between the bulky flesh where your male genitals protrude from between your hips and the " + player.buttDescript() + " above.");
					if (player.vaginas[0].vaginalWetness >= VaginaClass.WETNESS_DROOLING) //IF CHARACTER HAS A DROOLING PUSSY ADD SENTENCE
						outputText("  The desert sun beats down on your body, its fiery heat inflaming the senses of your vaginal lips.  Juices stream from your womanhood and begin pooling on the hot sand beneath you.");
				}
			}
			outputText("\n\n"); //PARAGRAPH 3
			outputText("You realize you are effectively trapped here by your own body.");
			...
			if (player.canFly()) //SCENE END = IF CHARACTER HAS FULL WINGS ADD SENTENCE
				outputText("  You extend your wings and flap as hard as you can, until at last you manage to lighten the bulk of your body somewhat - enough to allow yourself to drag your genitals across the hot sands and back to camp.  The ordeal takes nearly an hour.");
			else if (player.isTaur()) //SCENE END IF CHARACTER HAS CENTAUR BODY
