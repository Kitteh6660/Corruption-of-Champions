...
public function pcPenetratesKatDoubly():void {
	...
	if (player.cumQ() < 500)
		...
		if (wombShot) {
			...
			if (player.cor >= (75 - player.corruptionTolerance()) || (player.findPerk(PerkLib.Pervert) >= 0 && player.cor >= (33 - player.corruptionTolerance())) || player.findPerk(PerkLib.Sadist) >= 0) { //First time you have to be corrupt
...
private function bathTimePenetrated():void {
	if (!player.hasVagina()) { //Genderless or Male - get this out of the way since it will be a little different
		...
		if (flags[kFLAGS.KATHERINE_UNLOCKED] >= 4 && (player.cor >= (75 - player.corruptionTolerance())) || (player.findPerk(PerkLib.Pervert) >= 0 && player.cor >= (33 - player.corruptionTolerance())) || (player.findPerk(PerkLib.Sadist) >= 0 || doneSubmissive(KBIT_SUB_ORGASM_DENIAL))) {
