	...
	public class AmilyScene extends NPCAwareContent implements TimeAwareInterface
	{
		...
		*/
		...
		public function exploreVillageRuin():void {
			...
			clearOutput();
			//20% chance of ghost-girl
			if ((flags[kFLAGS.SHOULDRA_PALADIN_MAIDEN_COUNTDOWN] == 0 && rackCount() >= 2 && rand(5) == 0) && !followerShouldra() && flags[kFLAGS.SHOULDRA_FOLLOWER_STATE] != .5) {
			...
			// 33% chance of doll fight
			if (rand(3) == 0) {
				outputText("You pass by something large lying on the ground, and stop to take a look. Picking the object up, you find it is a rather large doll, as big as a child! It looks rather worn, so you decide to leave it and be on your way.\n\n...Except you can't seem to drop it. The doll starts to give off a strong aura, and you have a feeling you know what that aura is meant to do.\n\nThankfully, you are able to free a hand to take out your " + player.weaponName + ", as you prepare to attack!\n\n");
				startCombat(new Doll());
				return;
			}
