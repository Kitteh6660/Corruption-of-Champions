package classes.Scenes.NPCs {
	...
	public class Kiha extends Monster {
		...
		private function kihaFirePunch():void {
			...
			if (player.spe - spe > 0 && int(Math.random() * (((player.spe-spe) / 4) +80)) > 80) //Dodged
			...
			else if (player.findPerk(PerkLib.Evade) >= 0 && rand(100) < 10) //Determine if evaded
			...
			else if (player.findPerk(PerkLib.Misdirection) >= 0 && rand(100) < 10 && player.armorName == "red, high-society bodysuit") //"Misdirection"
			...
			else if (player.findPerk(PerkLib.Flexibility) >= 0 && rand(100) < 6) //Determine if cat'ed
		...
		private function kihaFireBreath():void {
			...
			if (player.findPerk(PerkLib.Evade) >= 0 && rand(100) < 10) //Determine if evaded
		...
		override protected function handleBlind():Boolean { return true; }
		...
		override public function defeated(hpVictory:Boolean):void {
			if (hasStatusEffect(StatusEffects.spiderfight)) game.kihaFollower.playerBeatsUpKihaPreSpiderFight();
			else if (hasStatusEffect(StatusEffects.DomFight)) game.kihaFollower.pcWinsDomFight();
			else if (hasStatusEffect(StatusEffects.Spar)) game.kihaFollower.winSparWithKiha();
			else game.kihaScene.kihaVictoryIntroduction();
		}
		override public function won(hpVictory:Boolean, pcCameWorms:Boolean):void {
			if (hasStatusEffect(StatusEffects.spiderfight)) game.kihaFollower.loseKihaPreSpiderFight();
			else if (hasStatusEffect(StatusEffects.DomFight)) game.kihaFollower.pcLosesDomFight();
			else if (hasStatusEffect(StatusEffects.Spar)) game.kihaFollower.sparWithFriendlyKihaLose();
			else if (pcCameWorms) {
				outputText("\n\nKiha seems visibly disturbed by your infection, enough that she turns to leave.");
				doNext(game.combat.endLustLoss);
			}
			else game.kihaScene.kihaLossIntro();
		}
		...
		public function Kiha() {
			...
			else this.long += "Her vagina constantly glistens with moisture, regardless of her state of arousal.  Despite her nudity, Kiha stands with the confidence and poise of a trained fighter.";
