	...
	//const KIHA_CHOKED_OUT_PC:int = 432;
		...
		public function encounterKiha():void {
			...
			if (flags[kFLAGS.KIHA_TOLL_DURATION] > 1) {
				kihaExplore(); //If currently paid up on toll, don't run into her!
				return;
			}
			flags[kFLAGS.TIMES_MET_KIHA]++; //count meetings
			if (player.isPureEnough(66) && flags[kFLAGS.KIHA_AFFECTION_LEVEL] == 0 && flags[kFLAGS.KIHA_TALK_STAGE] >= 3 && player.gender > 0) {
				kihaFollower.kihaSpiderEventIntro(); //PLOT FIGHT TIME!
				return;
			}
			...
			//Repeat Encounter - PC WAS VICTORIOUS LAST FIGHT
				...
				flags[kFLAGS.PC_WIN_LAST_KIHA_FIGHT] = 0; //clear 'won last fight' toggle
		...
		private function offerToBuyPassageFromKiha():void {
			...
			flags[kFLAGS.KIHA_TOLL] = 1; //unlocks toll option next encounter
			menu();
		...
		internal function meetKihaAndFight():void {
			...
			startCombat(new Kiha()); //START COMBAT
		}
		//Ask Why
		...
		private function payKihaTribute():void {
			...
			player.gems -= 200; //-200 gems
			flags[kFLAGS.KIHA_TOLL_DURATION] = 3 + rand(4);
			flags[kFLAGS.KIHA_TOLL]++; //count times paid
			//2-5 more explorations of her area before she shows up demanding more tribute!
			doNext(kihaExplore); //do a 'Kiha' exploration with chances of fantabulous prizes!
		}
		public function kihaExplore(clearScreen:Boolean = true):void {
			...
			if (event == 0 && flags[kFLAGS.GOTTEN_INQUISITOR_ARMOR] == 0) {
				inquisitorRobesDiscovery(); //grabbin' Inquisitor Armor
				return;
			}	
			if (event < 5) {
			...
			else if (event < 7) itype = consumables.REDUCTO; //reducto
		...
		internal function kihaVictoryIntroduction():void {
			...
			if (monster.lust >= monster.maxLust()) { //lust
				outputText("with lust as she tries to rise.\n\n");
				...
				outputText("weakly as she tries to rise.\n\n");
			...
			outputText("\n\nYou could forcibly masturbate her."); //for all
			//Girls
			menu();
			...
			//Conversation Only - Emotional Rape! (no more than 40 corruption!)
			if (player.isPureEnough(40)) {
		...
		*/
		...
		public function inquisitorRobesDiscovery():void {
			...
			if (player.inte < 60) {
				...
				doNext(camp.returnToCampUseOneHour); //player leaves, room can be re-encountered
				return;
			}
