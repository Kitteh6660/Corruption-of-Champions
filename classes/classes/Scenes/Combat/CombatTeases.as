package classes.Scenes.Combat 
{
	...
	public class CombatTeases extends BaseContent
	{
		...
		public function tease(justText:Boolean = false):void {
			...
			if (player.findPerk(PerkLib.Seduction) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) chance += 10;
			//10% for sexy armor types
			if (player.findPerk(PerkLib.SluttySeduction) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) chance += player.perkv1(PerkLib.SluttySeduction);
			//10% for bimbo shits
			if (player.findPerk(PerkLib.BimboBody) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) {
				chance += 10;
				bimbo = true;
			}
			if (player.findPerk(PerkLib.BroBody) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) {
				chance += 10;
				bro = true;
			}
			if (player.findPerk(PerkLib.FutaForm) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) {
				chance += 10;
				futa = true;
			}
			//2 & 2 for seductive valentines!
			if (player.findPerk(PerkLib.SensualLover) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) {
				chance += 2;
			}
			if (player.findPerk(PerkLib.ChiReflowLust) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) chance += UmasShop.NEEDLEWORK_LUST_TEASE_MULTI;
			...
			if (player.findPerk(PerkLib.Seduction) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) damage += 5;
			//+ slutty armor bonus
			if (player.findPerk(PerkLib.SluttySeduction) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) damage += player.perkv1(PerkLib.SluttySeduction);
			//10% for bimbo shits
			if (bimbo || bro || futa) {
				damage += 5;
				bimbo = true;
			...
			//==EXTRAS========
			//12 Cat flexibility.
			if (player.findPerk(PerkLib.Flexibility) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled) && player.isBiped() && player.hasVagina()) {
			...
			//14 Brood Mother
			if (monster.hasCock() && player.hasVagina() && player.findPerk(PerkLib.BroodMother) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled) && (player.pregnancyIncubation <= 0 || player.pregnancyIncubation > 216)) {
			...
			//27 FEEDER
			if (player.findPerk(PerkLib.Feeder) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled) && player.biggestTitSize() >= 4) {
			...
			switch(select) {
				...
				case 2:
					if (player.isTaur()) {
						...
						if (player.cocks.length > 0) {
							...
							if (player.findPerk(PerkLib.BulgeArmor) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) {
								damage += 5;
							}
							penis = true;
						}
				...
				case 3:
					if (player.isTaur() && player.countCocksOfType(CockTypesEnum.HORSE) > 0) {
						...
						if (player.findPerk(PerkLib.BulgeArmor) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) damage += 5;
					}
					else {
						outputText("You open your " + player.armorName + ", revealing your ", false);
						if (player.cocks.length == 1) outputText(player.cockDescript(0), false);
				...
				case 6:
					if (player.findPerk(PerkLib.BimboBrains) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled) || player.findPerk(PerkLib.FutaFaculties) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) {
						...
						else if (player.findPerk(PerkLib.BimboBody) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) outputText("bimbo ", false);
				...
				case 8:
					...
					if (player.findPerk(PerkLib.BroBrains) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) outputText("Damn, " + monster.a + monster.short + " has got to love this!", false);
				...
				case 9:
					...
					if (player.findPerk(PerkLib.BroBrains) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) outputText(monster.capitalA + monster.short + " can't resist such a heroic pose!", false);
				...
				case 10:
					...
					if (player.findPerk(PerkLib.BroBrains) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) outputText("No way could " + monster.pronoun1 + " resist your huge cock!", false);
				...
				case 11:
					if (silly() && rand(2) == 0) outputText("You strike a herculean pose and flex, whispering, \"<i>Do you even lift?</i>\" to " + monster.a + monster.short + ".", false);
						...
						if (player.findPerk(PerkLib.BroBrains) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) outputText("Bitches love a cum-leaking cock.", false);
			...
			if (rand(100) <= chance + rand(bonusChance)) {
				...
				if (player.findPerk(PerkLib.ChiReflowLust) >= 0 && !player.hasStatusEffect(StatusEffects.PerksDisabled)) damage *= UmasShop.NEEDLEWORK_LUST_TEASE_DAMAGE_MULTI;
