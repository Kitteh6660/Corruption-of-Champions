...
package classes.Scenes.Areas.GlacialRift {
	...
	public class ValkyrieScene extends BaseContent {
		...
		public function loseToValkyrie():void {
			clearOutput();
			outputText(images.showImage("valkyrie-defeat"));
		...
		private function loseToValkyrieForcedOral():void {
			...
			if (player.gender > 1) outputText(images.showImage("valkyrie-lose-oral-female"));
			else outputText(images.showImage("valkyrie-lose-oral-male"));
		...
		public function winAgainstValkyrie():void {
			...
			outputText(images.showImage("valkyrie-victory"));
			...
			//If Herm: All Options
			if (player.lust >= 33) {
				if (player.hasCock()) { //If Male: [Aerial Fuck] [Fuck Anal] [Leave]
					addButton(0, "Aerial Fuck", valkyrieAerialFuck);
					addButton(1, "Anal Fuck", valkyrieAnalFuck);
				}
				if (player.hasVagina()) { //If Female: [Get Licked] [Scissor]
		...
		private function valkyrieAerialFuck():void {
			...
			outputText(images.showImage("valkyrie-won-aerialfuck"));
		...
		private function valkyrieAnalFuck():void {
			...
			if (player.minoScore() > 4) outputText(images.showImage("valkyrie-won-assfuck-minotaur"));
			else if (player.biggestCockArea() > 65) outputText(images.showImage("valkyrie-won-assfuck-mighty"));
			else outputText(images.showImage("valkyrie-won-assfuck"));
		...
		private function valkyrieLicksYou():void {
			...
			if (player.vaginas[0].vaginalLooseness == VaginaClass.LOOSENESS_LEVEL_CLOWN_CAR && player.dragonScore() > 4) outputText(images.showImage("valkyrie-won-lickgina-dragon"));
			else outputText(images.showImage("valkyrie-won-lickgina"));
		...
		private function valkyrieScissor():void {
			...
			outputText(images.showImage("valkyrie-won-scissor"));
