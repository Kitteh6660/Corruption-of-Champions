...
package classes.Scenes.Areas.Mountain {
	...
	public class HellHoundScene extends BaseContent {
		...
		3. Catch the scent of their foe, lowering the chance of successfully running away;
			Raped by: Doggy style, using both dicks if the foe has two holes
			Rape: Upon defeat, the flames in the hellhound's mouth goes out. It submits to its foe by licking their gear */

		public function hellhoundEncounter():void {
			clearOutput();
			spriteSelect(SpriteDb.s_hellhound);
			outputText(images.showImage("encounter-hellhound"));
		...
		public function hellhoundRapesPlayer():void {
			...
			if (player.vaginas.length > 0 && player.lowerBody.type != LowerBody.NAGA) {
				if (player.cowScore() > 4) outputText(images.showImage("hellhound-loss-vag-cow"));
				else if (player.catScore() > 4) outputText(images.showImage("hellhound-loss-vag-feline"));
				else if (player.demonScore() > 4) outputText(images.showImage("hellhound-loss-vag-succubi"));
				...
				if (player.vaginas[0].virgin) outputText("<b>You are no longer a virgin!  </b>"); //lose player vaginal virginity
				...
				player.knockUp(PregnancyStore.PREGNANCY_HELL_HOUND, PregnancyStore.INCUBATION_HELL_HOUND, 101); //preggers chance!
				player.orgasm('Vaginal',false);
			}
			else {
				if (player.dogScore() > 4 || player.wolfScore() > 4) outputText(images.showImage("hellhound-loss-butt-canine"));
				else outputText(images.showImage("hellhound-loss-butt"));
			...
			player.buttChange(monster.cockArea(0), true); //lose player anal virginity
			player.orgasm('Anal');
			if (player.cor < 40) dynStats("tou", -2, "cor", 1); //if not corrupt
			else dynStats("cor", 1.5); //if corrupt
			combat.cleanupAfterCombat();
		}

		public function hellHoundGetsRaped():void {
			clearOutput();
			if (player.cocks.length >= 1) outputText(images.showImage("hellhound-won-cocksucker"));
			else if (player.gender == 2) outputText(images.showImage("hellhound-won-pussysucker"));
			else outputText(images.showImage("hellhound-won"));
			//Raped by player
			if (player.cocks.length == 1 && player.vaginas.length == 0) //if player has only one dick and no vagina
				 outputText("The twin heads of the hellhound eagerly lick your " + player.cockDescript(0) + " for a while.  The large, flat tongues give you a rather unusual feeling, but it is definitely effective. As you approach your peak, one of the heads tries to take your manhood into its mouth. But the other head doesn't seem all that pleased with this, and the two start to nip at each other in an effort to claim the right to be the one to push you over the edge. Annoyed at being ignored, you give both heads a smack with your hand. After a moment of recoil, they look down apologetically before resuming their licking. As you peak, the heads pull back and try to catch all the cum that erupts from your " + player.cockDescript(0) + " in their mouths. The fierce competition between the two means that most of the cum ends up outside their eager maws, but they are nonetheless satisfied.  With a pair of happy barks, the hellhound turns and runs away.");
			if (player.gender == 2) //if player has only a vagina and no dick
				 outputText("The twin heads of the hellhound eagerly lick your " + player.vaginaDescript(0) + " for a while.  The large, flat tongues give you a rather unusual feeling, but it is definitely effective. As you approach your peak, one of the heads tries to force the other out of the way and push its snout into your " + player.vaginaDescript(0) + ". But the other head doesn't seem all that pleased with this, and the two start to nip at each other in an effort to claim the right to be the one to push you over the edge. Annoyed at being ignored, you give both heads a smack with your hand. After a moment of recoil, they look down apologetically before resuming their licking. As you peak, the heads resume their quarrel and try to drink as much as the fluids coming from your " + player.vaginaDescript(0) + " as they can.  You don't know which got the most, but before long your " + player.vaginaDescript(0) + " is definitely quite clean.  With a pair of happy barks, the hellhound turns and runs away.");
			if (player.gender == 3 && player.cocks.length == 1) //if player has a dick and a vagina
			...
			if (player.cocks.length > 1) //if player has multiple endowments
			...
			if (player.gender == 0) //if player has no endowments
		...
		public function HellHoundMasterEncounter():void {
			...
			if (flags[kFLAGS.HELLHOUND_MASTER_PROGRESS] == 0) { //initial encounter (A)
				...
				doYesNo(acceptCrazyPresences, declineCrazyPresences); //player chooses yes/no - no -> (B), yes -> (C)
			}
			//Can be triggered if the PC still has the required canine traits, and has a piece of Lethicite
			else if (flags[kFLAGS.HELLHOUND_MASTER_PROGRESS] == 1) { //second encounter (D)
				...
				doYesNo(giveUpLethicite, declineGivingAwayLethicite); //player chooses yes/no, - no -> (E), yes -> (F)
			}
		}
		//Choose no (B)
		private function declineCrazyPresences():void {
			...
			doNext(camp.returnToCampUseOneHour); //end event, (A) can repeat later
		}
		//Choose yes (C)
		private function acceptCrazyPresences():void {
			clearOutput();
			outputText(images.showImage("hellhound-hellhound'sDen"));
			...
			doNext(acceptCrazyPresencesII); //next page
		}
		private function acceptCrazyPresencesII():void {
			...
			flags[kFLAGS.HELLHOUND_MASTER_PROGRESS] = 1; //advance to repeat version
		...
		private function declineGivingAwayLethicite():void {
			...
			doNext(camp.returnToCampUseOneHour); //end event, (D) can repeat
		}
		//Choose yes (F)
		private function giveUpLethicite():void {
			clearOutput();
			outputText(images.showImage("hellhound-hellhound'sDen-II"));
			...
			//Player chooses to either give Merae's full Lethicite, or a regular piece of Lethicite. Limited by what they have, of course. They cannot choose to leave at this point
			menu();
			if (player.keyItemv2("Marae's Lethicite") == 0) //Merae's Lethicite -> (G)
				 addButton(0, "Give All", giveALLTHELETHICITES);
			else addDisabledButton(0, "Give All");
			addButton(1, "Give Part", giveLethicitePiece); //Regular Lethicite -> (H)
		}
		//Give a single piece of Lethicite, get hellfire (H)
		private function giveLethicitePiece():void {
			...
			if (player.keyItemv1("Marae's Lethicite") == 2) { //if PC gave 2/3 Merae's crystal
				...
				player.addKeyValue("Marae's Lethicite", 1, -1); //replace 2/3 Merae's Lethicite with regular lethicite
			}
			else { //remove 1 lethicite crystal from the PC
			...
			doNext(giveLethicitePieceEnd);
		}
		private function giveLethicitePieceEnd():void {
			clearOutput();
			...
			dynStats("lib", 3, "lus", 30, "cor", 6); //increase lust by 30, corruption by 6, and libido by 3
			if (player.findPerk(PerkLib.Hellfire) < 0) //Player gains hellfire perk. Hellfire deals physical damage to completely pure foes, lust damage to completely corrupt foes, and a mix for those in between. Its power is based on the PC's corruption and level. Appearance is slightly changed to mention that the PC's eyes and mouth occasionally show flicks of fire from within them, text could possibly vary based on corruption
				player.createPerk(PerkLib.Hellfire, 0, 0, 0, 0);
			flags[kFLAGS.HELLHOUND_MASTER_PROGRESS]++; //hellhounds no longer encounterable
			doNext(camp.returnToCampUseOneHour);
		}
		//Give him Merae's Lethicite, now he's got enough power for a major upheaval in the demon hierarchy. You don't get to keep being a champion (G)
		private function giveALLTHELETHICITES():void {
			clearOutput();
			dynStats("lus=", player.maxLust()); //max player's lust for genital descriptions
			...
			if (player.dogCocks() >= 2) { //if PC has two dog dicks or more: the male bad end
				...
				if (player.totalCocks() > 2 || player.cockThatFits(60) < 0) { //make sure that the PC will fit in the bitch (PC's dog cocks are not each of an area of 20 to 60 OR PC has more than two cocks)
					...
					while (player.cocks.length > 2) player.removeCock(2, 1);
				}
				outputText(images.showImage("hellhound-badend-male"));
				...
				outputText("\"<i>Well my new pet, what do you think?</i>\"  A voice calls out in your head.  You look to the side, and see one of Cremera's heads on your right, and the other on your left.  Your hands seem to move on their own, and grip your twin dicks.");
			}
			else { //female part
				...
				outputText(images.showImage("hellhound-badend-female"));
				...
				if (player.vaginalCapacity() < 45) { //if PC's vag can't fit an area of 45
					if (player.vaginas[0].virgin) outputText("Relieving both your virgin holes of their virginity in one thrust, y");
					else outputText("Y");
					player.vaginas[0].vaginalLooseness = VaginaClass.LOOSENESS_GAPING; //stretch vag to very loose
				...
				outputText("<i>Well my new pet, what do you think?</i>\"  A voice calls out in your head.  You look to the side, and see one of Cimer's heads on your right, and the other on your left.  Your hands seem to move on their own, and reach down to touch your " + player.vaginaDescript(0) + ".");
			}
			outputText("  Another voice calls out in your mind, <i>They are wonderful master.</i>\"");
			doNext(HellhoundBadEnd);
		}
		private function HellhoundBadEnd():void {
			clearOutput();
			spriteSelect(SpriteDb.s_hellhound);
			outputText(images.showImage("badend-hellhound"));
		...
		public function hellHoundPropahRape():void {
			...
			if (player.isPureEnough(20)) outputText(images.showImage("hellhound-won-pure"));
			...
			if (player.isPureEnough(20)) { //if corruption < 20
				...
				if (player.isPureEnough(40) && player.findPerk(PerkLib.Masochist) < 0) { //corruption < 40 (and not masocistic - I lost track if there is such a perk)
					outputText(images.showImage("hellhound-won-vagfucker-pure"));
				...
				//Else corruption >= 40
				if (player.isGoo()) outputText(images.showImage("hellhound-won-vagfucker-goo"));
				else if (player.cowScore() > 4) outputText(images.showImage("hellhound-won-vagfucker-cow"));
				else if (player.demonScore() > 4) outputText(images.showImage("hellhound-won-vagfucker-succubi"));
				else if (player.dogScore() > 4 || player.wolfScore() > 4) outputText(images.showImage("hellhound-won-vagfucker-canine"));
				else if (player.isPregnant()) outputText(images.showImage("hellhound-won-vagfucker-preg"));
				else outputText(images.showImage("hellhound-won-vagfucker"));
				...
				if (player.hasCock()) { //if PC has penis
				...
				if (player.totalCocks() > 0) { //if PC has penis
					...
					if (player.cumQ() > 700) //if messy orgasm perk or cum multiplier > 5
				...
				if (player.isPureEnough(40)) dynStats("tou", -2, "cor", 1); //if not corrupt
				else dynStats("cor", 1.5); //if corrupt
				player.knockUp(PregnancyStore.PREGNANCY_HELL_HOUND, PregnancyStore.INCUBATION_HELL_HOUND, 101); //preggers chance!
			}
			combat.cleanupAfterCombat();
