package classes.Scenes.Areas.Mountain {
	...
	public class WormMass extends Monster {
		public function wormAttack():void {
			...
			if (player.findPerk(PerkLib.Evade) >= 0 && rand(100) < 10) { //evade
		...
		public function wormsEntice():void {
			clearOutput();
			outputText(images.showImage("wormmass-entice"));
			if (rand(2) == 0) { //FAIL
			...
			if (player.lust100 < 50) {
				...
				if (player.hasStatusEffect(StatusEffects.InfestAttempted)) { //chance of insta-loss if infested twice
		...
		override public function won(hpVictory:Boolean, pcCameWorms:Boolean):void {
			clearOutput();
			outputText(images.showImage("wormmass-loss"));
		...
		override public function maxHP():Number { return 40 + (player.newGamePlusMod() * 20); }
