package classes.Scenes.Areas.Mountain {
	...
	public class Salon extends BaseContent implements TimeAwareInterface {
		...
		public function Salon() { CoC.timeAwareClassAdd(this); }
		...
		public function hairDresser():void {
			clearOutput();
			outputText(images.showImage("event-mysterious-cave"));
		...
		public function isDiscovered():Boolean { return player.hasStatusEffect(StatusEffects.HairdresserMeeting); }
		...
		private function salonFavoritesPaymentMenu():void {
			...
			if (player.hasItem(consumables.MINOCUM) || player.hasItem(consumables.P_M_CUM) || player.hasItem(consumables.URTACUM) || player.hasItem(consumables.BHMTCUM))
				 addButton(9, "Give Cum", giveCumOptions);
		...
		private function buyMinoCum():void {
			...
			if (player.gems < 60) {
				...
				if (flags[kFLAGS.SALON_PAID] == 0) doNext(salonGreeting);
				else salonPurchaseMenu();
			}
			else {
				player.gems -= 60;
				outputText(images.showImage("item-bottle-cum"));
		...
		private function hairDresserRepeatGreeting():void {
			...
			if (flags[kFLAGS.LYNNETTE_PREGNANCY_CYCLE] == 4 && flags[kFLAGS.LYNNETTE_CARRYING_COUNT] > 0) {
				...
				if (flags[kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL] == 1) { //if favorite!
					favoriteSalonMenu();
					return;
				}
				else {
			...
			//Favorite Announcement:
				...
				outputText(images.showImage("salon-lynnette-fav"));
				...
				outputText("This seems like quite the deal!");
				flags[kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL] = 1;
				favoriteSalonMenu(); //custom menu with options to "pay anyway"
				return;
			}
			//Favorite Greeting:
			else if (flags[kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL] == 1) {
				if (flags[kFLAGS.LYNNETTE_PREGNANCY_CYCLE] < 4) {
					outputText(images.showImage("salon-lynnette-preg"));
					if (flags[kFLAGS.LYNNETTE_CARRYING_COUNT] > 0) //Pregnant With Your Babies
					...
					else //Pregnant With Other Babies
			...
			//Standard Repeats:
				...
				else outputText("Will you be getting our normal services?  Just one BJ per hair treatment!  Or would you rather get it from the gloryholes in back?</i>\"");
			}
			salonPaymentMenu();
		}
		...
		private function gloryholeImp():void {
			clearOutput();
			outputText(images.showImage("salon-gloryhole"));
		...
		private function giveCumToLynnette(type:int):void {
			...
			if (type == 0) { //Minotaur Cum
				...
				else player.destroyItems(consumables.P_M_CUM, 1);
			}
		...
		private function goblinHairDresserFacefuck():void {
			clearOutput();
			spriteSelect(SpriteDb.s_lynette);
			outputText(images.showImage("salon-lynnette-blowjob"));
		...
		private function cutShort():void {
			clearOutput();
			spriteSelect(SpriteDb.s_lynette);
			if (player.hair.type == 4) { //tentacle hair
				outputText(images.showImage("location-salon"));
			...
			outputText(images.showImage("salon-work-hair"));
		...
		private function removeHair():void {
			clearOutput();
			outputText(images.showImage("salon-remove-hair"));
		...
		private function dyeMenu():void {
			spriteSelect(SpriteDb.s_lynette);
			clearOutput();
			outputText(images.showImage("salon-cabinet"));
		...
		private function cutBeard():void {
			clearOutput();
			if (player.hair.type == 4) { //tentacle beard
				outputText(images.showImage("salon-lynnette"));
				outputText("Lynnette stares at you when you ask for a cut.  \"<i>Nothing doing, hon; that stuff looks alive and I don't want blood all over my nice floor.  Thanks for contributing to the white file, though; maybe we can do something else?</i>\"\n\n");
				beardMenu();
				return;
			}
			if (player.mf("m", "f") == "f") outputText(images.showImage("salon-work-beard-female"));
			else outputText(images.showImage("salon-work-beard-male"));
		...
		private function growBeard(mode:int = 0):void {
			clearOutput();
			if (mode == 1) { //grow beard if you don't have
				if (player.mf("m", "f") == "f") {
					outputText(images.showImage("salon-work-beard-female"));
					...
					outputText(images.showImage("salon-work-beard-male"));
		...
		private function changeBeardStyle():void {
			clearOutput();
			outputText(images.showImage("salon-work-beard"));
		...
		private function removeBeard():void {
			clearOutput();
			outputText(images.showImage("salon-remove-beard"));
		...
		private function minotaurCumBukkakeInSalon():void {
			...
			if (player.gender > 1) {
				if (player.gender == 3 && !player.hasCock()) outputText(images.showImage("salon-minobukkake-cuntboy"));
				else outputText(images.showImage("salon-minobukkake-female"));
			}
			else outputText(images.showImage("salon-minobukkake-male"));
			...
			else outputText("You crush the member tightly against your chest, smearing your torso with the copious pre until you're like a hot, wet slip-n-slide of minotaur sex.  You make up for it by wrapping your arms around his prodigious thickness and squeeze him tightly with your hands, stroking him off with your whole body with enough tightness to make the poor mino' think he's in some poor adventurer's asshole.");
			outputText("\n\nYou lower and raise yourself, bouncing up and down on your " + player.legs() + " to enhance the ");
			...
			if (player.hasVagina()) {
				...
				//ADD PREG CHECK
				player.knockUp(PregnancyStore.PREGNANCY_MINOTAUR, PregnancyStore.INCUBATION_MINOTAUR, 70); //preggers chance!
			}
			outputText(".\n\n");
			...
			outputText("\n\nThe strength goes completely out of your " + player.legs() + ", but you manage to hold yourself up long enough for your stud to flex his cock inside you and fill up your backdoor with more potent addiction.  You slide off, nerveless and still orgasming as jizz rains on your back from the abandoned cock, rolling off you to add to the ever-widening puddle of fluids on the floor.  Smiling vacantly and addled on more minotaur-cum than any one person has reason to handle, you stumble up on all fours and crawl to the next one.  More...");
			player.minoCumAddiction(10);
			player.orgasm('Lips',false);
			player.orgasm('Anal');
			player.slimeFeed();
			dynStats("lib", 2, "sen", 2, "cor", 2);
			doNext(minotaurSalonFollowUp); //next
		}
		private function minotaurSalonFollowUp():void {
			...
			outputText(images.showImage("location-salon"));
			if (flags[kFLAGS.SALON_MINOCUM_UNLOCKED] == 0) {
				flags[kFLAGS.SALON_MINOCUM_UNLOCKED]++; //unlock mino cum purchase
			...
			outputText("Lynnette offers and explains their options, \"<i>So what'll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>\"");
			salonPurchaseMenu(); //menu
		}

		private function mudFacial():void {
			clearOutput();
			spriteSelect(SpriteDb.s_lynette);
			outputText(images.showImage("salon-work-face"));
		...
		/*
		public static const LYNNETTE_PREGNANCY_CYCLE:int = 1022; //pregnant (0-3), not (4-6)
		...
		*/
		...
		private function fuckLynnette():void {
			clearOutput();
			spriteSelect(SpriteDb.s_lynette);
			if (flags[kFLAGS.LYNNETTE_FUCK_COUNTER] == 0) { //checks to see if you've cum withint the past 24 hours
				if (player.hoursSinceCum < 24) outputText(images.showImage("salon-lynnette"));
				...
				outputText(".");
				if (player.hoursSinceCum < 24) { //IF FAIL!
			...
			else { //repeat cum within 24 hours check
				if (player.hoursSinceCum < 24) outputText(images.showImage("salon-lynnette"));
				...
				outputText(".");
				if (player.hoursSinceCum < 24) { //NOT ENOUGH!
			...
			outputText("\"<i>Fuck me,</i>\" the pregnancy-obsessed slut begs as she moves a hand down to her cunny, spreading the lips wide-open with reveal a glittering amethyst tunnel that promises sublime pleasure to all who enter.");
			dynStats("lus", 100);
			doNext(fuckLynnetteToTheSky);
		}
		private function fuckLynnetteToTheSky():void {
			clearOutput();
			spriteSelect(SpriteDb.s_lynette);
			if (player.countCocksOfType(CockTypesEnum.TENTACLE) > 1) outputText(images.showImage("salon-lynnette-fuck-tentacle"));
			else if (player.countCocksOfType(CockTypesEnum.ANEMONE) >= 1) outputText(images.showImage("salon-lynnette-fuck-anemone"));
			else outputText(images.showImage("salon-lynnette-fuck"));
			...
			if (player.cumQ() < 1000) { //BELOW ONE LITER!
			...
			else if (player.cumQ() <= 10000) { //"GOOD"
				...
				else /* "GOOD + LYNNETTE FAVORITE" */ outputText("\"<i>Mmmm, [name], I love the way you pack me full.</i>\"  She tosses back a vial of fertility-boosting chemicals and sighs.");
				lynnetteApproval(10);
			}
			else { //"UBUR"
			...
			flags[kFLAGS.LYNNETTE_PREGNANCY_CYCLE] = 0;
			doNext(fuckLynnetteEnd);
		}
		private function fuckLynnetteEnd():void {
			clearOutput();
			spriteSelect(SpriteDb.s_lynette);
			outputText(images.showImage("location-salon"));
			if (player.cumQ() < 1000) outputText("You leave her feeling a little underappreciated.  Maybe you could find some cum-boosting supplements out there?  Still, you are owed a haircut...");
			else if (player.cumQ() <= 10000) {
				if (flags[kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL] == 1) outputText("You leave with a happy smile. It seems Lynnette approves of you as a mate.");
				else outputText("\"<i>Do me a favor and come back soon.  I'd rather be pouring out swarms of your babies than a slurry of weakling sluts.</i>\"");
			}
			else {
				if (flags[kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL] == 0) outputText("You swagger out into the main room with a happy smile, noting the envious looks you get from her daughters when they try to offer you 'services'.");
				else outputText("You silence her with a kiss that leaves her swooning and stroll out, dressing on the way.  Lynnette's satiated moans seem to haunt you the entire time you remain in the salon.");
			}
			salonPurchaseMenu();
		}
