	...
	public class Factory extends DungeonAbstractContent {
		...
		*/
		...
		public function enterDungeon():void {
			...
			if (flags[kFLAGS.FACTORY_SHUTDOWN] == 2) //Shutdown state
		...
		private function doTensionRelease():void {
			...
			if (!player.hasStatusEffect(StatusEffects.TensionReleased)) { //First time...
				...
				if (player.biggestTitSize() < 10) player.growTits(1, (2+rand(3)), true, 1);
				...
				if (player.statusEffectv1(StatusEffects.TensionReleased) == 0) { //Second time...
						if (player.hasVagina() && player.hasCock()) outputText(images.showImage("factory-tension-herm"));
						else if (player.hasVagina() && !player.hasCock()) outputText(images.showImage("factory-tension-female"));
						else outputText(images.showImage("factory-tension-male"));
				...
				else //Third time, move on to bad end!
					doBadEndTension();
			}
			doNext(roomMainChamber);
		}
		...
		private function doBadEndTension2():void {
			clearOutput();
		...
		public function roomFurnaceRoom():void {
			...
			outputText("Despite the machinery being shut down, the air in this room is still hot enough to coat your " + player.skin.tone + " " + player.skin.desc + " in a fine sheen of sweat.  The eastern side of the chamber is more machine than wall, a solid mass of iron piping covered in small metal blast-doors through which fuel is to be fed.  A small transparent plate is riveted into the wall, allowing you to see some ");
			if (flags[kFLAGS.FACTORY_SHUTDOWN] <= 0) outputText("kind of pink crystalline fuel being burned by purple-white fire.");
			else outputText("the ashes of a previous fuel source.")
			outputText(" The few visible controls and gauges don't seem to be linked into anything important, and the machinery looks far too durable to damage with what you have.  The only exit is a heavy iron door on the west wall.  ");
		...
		public function roomForemanOffice():void {
			...
			if (flags[kFLAGS.FACTORY_OMNIBUS_DEFEATED] <= 0) omnibusOverseer.encounterOmnibus();
			else {
				if (player.hasKeyItem("Supervisor's Key") < 0) addButton(0, "Desk", takeSupervisorKey).hint("Check the desk for something.");
			}
		}
