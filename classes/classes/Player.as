package classes {
	...
	public class Player extends PlayerHelper {
		...
		protected function outputText(text:String):void { game.outputText(text); }
		...
		//Lust vulnerability
		public var lustVuln:Number = 1; /**@TODO: kept for backwards compatibility reasons but should be phased out*/
		//Teasing attributes
		...
		override public function pregnancyUpdate():Boolean { return game.pregnancyProgress.updatePregnancy(); } //returns true if we need to make sure pregnancy texts aren't hidden
		//Inventory
		...
		override public function set armorName(value:String):void { CoC_Settings.error("ERROR: attempt to directly set armorName."); }
		override public function set armorPerk(value:String):void { CoC_Settings.error("ERROR: attempt to directly set armorPerk."); }
		override public function set armorValue(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set armorValue."); }
		override public function set armorDef(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set armorDef."); }
		//override public function set weapons
		override public function set weaponName(value:String):void { CoC_Settings.error("ERROR: attempt to directly set weaponName."); }
		override public function set weaponPerk(value:String):void { CoC_Settings.error("ERROR: attempt to directly set weaponPerk."); }
		override public function set weaponValue(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set weaponValue."); }
		override public function set weaponAttack(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set weaponAttack."); }
		override public function set weaponVerb(value:String):void { CoC_Settings.error("ERROR: attempt to directly set weaponVerb."); }
		//override public function set jewelries
		override public function set jewelryName(value:String):void { CoC_Settings.error("ERROR: attempt to directly set jewelryName."); }
		override public function set jewelryPerk(value:String):void { CoC_Settings.error("ERROR: attempt to directly set jewelryPerk."); }
		override public function set jewelryValue(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set jewelryValue."); }
		override public function set jewelryEffectId(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set jewelryEffectId."); }
		override public function set jewelryEffectMagnitude(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set jewelryEffectMagnitude."); }
		//override public function set shields
		override public function set shieldName(value:String):void { CoC_Settings.error("ERROR: attempt to directly set shieldName."); }
		override public function set shieldPerk(value:String):void { CoC_Settings.error("ERROR: attempt to directly set shieldPerk."); }
		override public function set shieldValue(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set shieldValue."); }
		override public function set shieldBlock(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set shieldBlock."); }
		//override public function set undergarments
		override public function set upperGarmentName(value:String):void { CoC_Settings.error("ERROR: attempt to directly set upperGarmentName."); }
		override public function set upperGarmentPerk(value:String):void { CoC_Settings.error("ERROR: attempt to directly set upperGarmentPerk."); }
		override public function set upperGarmentValue(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set upperGarmentValue."); }
		override public function set lowerGarmentName(value:String):void { CoC_Settings.error("ERROR: attempt to directly set lowerGarmentName."); }
		override public function set lowerGarmentPerk(value:String):void { CoC_Settings.error("ERROR: attempt to directly set lowerGarmentPerk."); }
		override public function set lowerGarmentValue(value:Number):void { CoC_Settings.error("ERROR: attempt to directly set lowerGarmentValue."); }
		...
		override public function get armorPerk():String { return _armor.perk; }
		override public function get armorValue():Number { return _armor.value; }
		override public function get armorDef():Number {
			...
			//Skin armor perk
			if (hasPerk(PerkLib.ThickSkin)) armorDef += 2;
			...
			if (armor == game.armors.SAMUARM && weapon == game.weapons.KATANA0) armorDef += 2;
			...
			if (hasPerk(PerkLib.Juggernaut) && tou >= 75 && armorPerk == "Heavy") speedBonus += 10;
			armorDef += speedBonus
			//Acupuncture effect
		...
		public function get armorBaseDef():Number { return _armor.def; }
		//override public function get weapons
		override public function get weaponName():String { return _weapon.name; }
		override public function get weaponPerk():String { return _weapon.perk || ""; }
		override public function get weaponValue():Number { return _weapon.value; }
		override public function get weaponVerb():String { return _weapon.verb; }
		override public function get weaponAttack():Number {
			var attack:Number = _weapon.attack;
			if (hasPerk(PerkLib.WeaponMastery) && weaponPerk == "Large" && str > 60) attack *= 2;
			if (hasPerk(PerkLib.LightningStrikes) && spe >= 60 && weaponPerk != "Large") attack += Math.round((spe - 50) / 3);
			//Iron fists bonus!
			...
			if (armor == game.armors.SAMUARM && weapon == game.weapons.KATANA0) attack += 2;
			//Berserking bonus!
		...
		public function get weaponBaseAttack():Number { return _weapon.attack; }
		//override public function get jewelries
		override public function get jewelryName():String { return _jewelry.name; }
		override public function get jewelryPerk():String { return _jewelry.perk; }
		override public function get jewelryValue():Number { return _jewelry.value; }
		override public function get jewelryEffectId():Number { return _jewelry.effectId; }
		override public function get jewelryEffectMagnitude():Number { return _jewelry.effectMagnitude; }
		//override public function get shields
		override public function get shieldName():String { return _shield.name; }
		override public function get shieldPerk():String { return _shield.perk; }
		override public function get shieldValue():Number { return _shield.value; }
		...
		override public function get upperGarmentName():String { return _upperGarment.name; }
		override public function get upperGarmentPerk():String { return _upperGarment.perk; }
		override public function get upperGarmentValue():Number { return _upperGarment.value; }
		public function get upperGarment():Undergarment { return _upperGarment; }
		
		override public function get lowerGarmentName():String { return _lowerGarment.name; }
		override public function get lowerGarmentPerk():String { return _lowerGarment.perk; }
		override public function get lowerGarmentValue():Number { return _lowerGarment.value; }
		public function get lowerGarment():Undergarment { return _lowerGarment; }
		...
		public function setArmorHiddenField(value:Armor):void { this._armor = value; } //in case you don't want to call the value.equip
		...
		public function setWeaponHiddenField(value:Weapon):void { this._weapon = value; } //in case you don't want to call the value.equip

		public function get jewelry():Jewelry { return _jewelry; } //jewelry, added by Kitteh6660
		...
		public function setJewelryHiddenField(value:Jewelry):void { this._jewelry = value; } //in case you don't want to call the value.equip
		...
		public function setShieldHiddenField(value:Shield):void { this._shield = value; } //in case you don't want to call the value.equip
		...
		public override function takeDamage(damage:Number, display:Boolean = false):Number {
			damage = Math.round(damage); //round
			var returnDamage:int = (damage > 0 && damage < 1) ? 1:damage; //we return "1 damage received" if it is in (0..1) but deduce no HP
			if (damage > 0) {
				...
				if (flags[kFLAGS.MINOTAUR_CUM_REALLY_ADDICTED_STATE] > 0) game.dynStats("lus", int(damage / 2));
				//Prevent negatives
				if (HP <= 0) HP = 0; //This call did nothing. There is no event 5010: if (game.inCombat) kGAMECLASS.output.doNext(5010);
			}
			return returnDamage;
		}

		public override function takeLustDamage(lustDmg:Number, display:Boolean = true, applyRes:Boolean = true):Number {
			lustDmg = Math.round(lustDmg); //round
			var lust:int = game.player.lust;
			var returnlustDmg:int = (lustDmg > 0 && lustDmg < 1) ? 1:lustDmg; //we return "1 damage received" if it is in (0..1) but deduce no Lust
			if (lustDmg > 0) {
		...
		public function bodyType():String {
			...
			else if (thickness < 25) { //Pretty thin
			...
			else if (thickness < 40) { //Somewhat thin
		...
		override public function get race():String {
			...
			if (catScore() >= 4) {
				if (isTaur() && lowerBody.type == LowerBody.CAT) {
					race = "cat-taur";
					if (face.type == 0) race = "sphinx-morph"; //no way to be fully feral anyway
				}
				else {
					race = "cat-morph";
					if (face.type == 0) race = "cat-" + mf("boy", "girl");
				}
			}
			if (lizardScore() >= 4) {
				...
				if (isTaur()) race += "-taur";
				if (lizardScore() >= 9) //High lizardScore? Always return lizan-race
					return race;
			}
			if (dragonScore() >= 6) {
				race = "dragon-morph";
				if (face.type == 0) race = "dragon-" + mf("man", "girl");
				if (isTaur()) race = "dragon-taur";
			}
			if (cockatriceScore() >= 4) {
				race = "cockatrice-morph";
				if (cockatriceScore() >= 8) race = "cockatrice";
				if (face.type == 0) race = "cockatrice-" + mf("boy", "girl");
			...
			if (sheepScore() >= 4) {
				if (lowerBody.legCount == 4 && lowerBody.type == 21) race = "sheep-taur";
				else if (gender == 0 || gender == 3) race = "sheep-morph";
				else if (gender == 1 && horns.type == 10) race = "ram-morph";
				else race = "sheep-" + mf("boy","girl");
			}
			if (wolfScore() >= 4) {
				if (hasFur() || gender == 0 || gender == 3)
					 race = "wolf-morph";
				else race = "wolf-" + mf("boy","girl");
			}
			if (dogScore() >= 4) {
				if (isTaur() && lowerBody.type == LowerBody.DOG) race = "dog-taur";
				else {
					race = "dog-morph";
					if (face.type == 0) race = "dog-" + mf("man", "girl");
				}
			}
			if (foxScore() >= 4) {
			...
			if (kitsuneScore() >= 4) race = "kitsune";
			if (horseScore() >= 3) {
			...
			if (cowScore() > 5) race = "cow-" + mf("morph", "girl");
		...
		//<mod>
			if (pigScore() >= 4) {
				race = "pig-morph";
				if (face.type == 0) race = "pig-" + mf("boy", "girl");
				if (face.type == 20) race = "boar-morph";
			}
			if (satyrScore() >= 4) race = "satyr";
			if (rhinoScore() >= 4) {
				race = "rhino-morph";
			...
			if (dragonneScore() >= 6) {
				if (isTaur()) race = "dragonne-taur";
					...
					race  = "dragonne-morph";
					if (face.type == 0) race = "dragonne-" + mf("boy", "girl");
				}
			}
			if (manticoreScore() >= 6) {
				race = "manticore-morph"
				if (face.type == 0) race = "manticore-" + mf("boy", "girl");
			}
			if (sirenScore() >= 4) race = "siren";
		//</mod>
			if (lowerBody.type == 3)race = "naga";
			...
			if (impScore() >= 4) race = "imp";
			return race;
		}
		//Determine Red Panda Rating
		...
		public function impScore():Number {
			...
			if (tallness > 42) impCounter--;
			if (biggestTitSize() > 0) impCounter--;
			if (bRows() == 2) impCounter--; //each extra row takes off a point
			if (bRows() == 3) impCounter -= 2;
			if (bRows() == 4) impCounter -= 3; //if you have more than 4 why are trying to be an imp
			return impCounter;
		}
		...
		public function demonScore():Number {
			...
			if (face.type == 0 && cor > 50) demonCounter++;
		...
		public function get minotaurScore():Number { return this.minoScore(); }
		...
		public function wolfScore():Number {
			...
			if (hasFur() && wolfCounter > 0) wolfCounter++; //only counts if we got wolf features
			if (wolfCounter >= 2) {
		...
		public override function dogScore():Number {
			...
			if (hasFur() && dogCounter > 0) dogCounter++; //fur only counts if some canine features are present
			if (dogCounter >= 2) {
		...
		public function mouseScore():Number {
			...
			if (hasFur() && coonCounter > 0) coonCounter++; //fur only counts if some coon features are present
		...
		public override function foxScore():Number {
			...
			if (hasFur() && foxCounter > 0) foxCounter++; //fur only counts if some canine features are present
			return foxCounter;
		}
		//Determine Cat Rating
		public function catScore():Number {
			var catCounter:Number = 0;
			if (face.type == Face.CAT) catCounter++;
			...
			if (hasFur() && catCounter > 0) catCounter++; //fur only counts if some cat features are present
			return catCounter;
		}
		...
		public function lizardScore():Number {
			...
			if (arms.type == Arms.PREDATOR && arms.claws.type == Claws.LIZARD) lizardCounter++;
			if (lizardCounter > 2) {
		...
		public function horseScore():Number {
			...
			if (hasFur() && horseCounter > 0) horseCounter++; //fur only counts if some equine features are present
			return horseCounter;
		}
		...
		public function kitsuneScore():Number {
			...
			if (tail.type == Tail.FOX && tail.venom >= 2) kitsuneCounter += 2; //if the character has two or more fox tails, +2
			//If the character has tattooed skin, +1
			//9999
			if (vaginalCapacity() >= 8000) kitsuneCounter++; //if the character has a 'vag of holding', +1
			//If the character's kitsune score is greater than 0 and:
			if (kitsuneCounter > 0 && (face.type == Face.HUMAN || face.type == Face.FOX)) kitsuneCounter++; //if the character has a normal face, +1
			//If the character's kitsune score is greater than 1 and:
			if (kitsuneCounter > 0 && (InCollection(hairOrFurColors, convertMixedToStringArray(ColorLists.BASIC_KITSUNE_HAIR))
				|| InCollection(hairOrFurColors, ColorLists.ELDER_KITSUNE))) kitsuneCounter++; //if the character has "blonde","black","red","white", or "silver" hair, +1
			if (kitsuneCounter > 0 && femininity >= 40) kitsuneCounter++; //if the character's femininity is 40 or higher, +1
			if (hasFur() && !InCollection(hairOrFurColors, convertMixedToStringArray(ColorLists.BASIC_KITSUNE_FUR))
				&& !InCollection(hairOrFurColors, ColorLists.ELDER_KITSUNE)) kitsuneCounter--; //if the character has fur, -1
			if (hasScales()) kitsuneCounter -= 2; //if the character has scales, -2
			if (hasGooSkin()) kitsuneCounter -= 3; //if the character has fur gooey skin, -3
			if (lowerBody.type != LowerBody.HUMAN && lowerBody.type != LowerBody.FOX) kitsuneCounter--; //if the character has abnormal legs, -1
			if (face.type != Face.HUMAN && face.type != Face.FOX) kitsuneCounter--; //if the character has a nonhuman face, -1
			if (ears.type != Ears.FOX) kitsuneCounter--; //if the character has ears other than fox ears, -1
			if (tail.type != Tail.FOX) kitsuneCounter--; //if the character has tail(s) other than fox tails, -1
		...
		public function dragonScore():Number {
			...
			if (arms.type == Arms.PREDATOR && arms.claws.type == Claws.DRAGON) dragonCounter++;
			if (eyes.type == Eyes.DRAGON) dragonCounter++;
			if (hasDragonNeck()) dragonCounter++;
			if (hasDragonRearBody()) dragonCounter++;
			return dragonCounter;
		}
		//Determine Goblin Rating
		public function goblinScore():Number {
			var horseCounter:Number = 0;
			if (ears.type == Ears.ELFIN) horseCounter++;
			if (ColorLists.GOBLIN_SKIN.indexOf(skin.tone) !== -1) horseCounter++;
			if (horseCounter > 0) {
				if (face.type == Face.HUMAN) horseCounter++;
				if (tallness < 48) horseCounter++;
				if (hasVagina()) horseCounter++;
				if (lowerBody.type == LowerBody.HUMAN) horseCounter++;
			}
			return horseCounter;
		}
		//Determine Goo Rating
		...
		public function sharkScore():Number {
			...
			if (hasPlainSkin() && sharkCounter > 0) sharkCounter++; //skin counting only if PC got any other shark traits
			return sharkCounter;
		}
		...
		public function salamanderScore():Number {
			...
			if (hasPerk(PerkLib.Lustzerker)) salamanderCounter++;
			if (salamanderCounter >= 2) {
				if (countCocksOfType(CockTypesEnum.LIZARD) > 0) salamanderCounter++;
				if (face.type == 0) salamanderCounter++;
				if (ears.type == 0) salamanderCounter++;
			}
			return salamanderCounter;
		}
		//-----<Mod-Added>-----
		//Determine Siren Rating
		public function sirenScore():Number  {
			var sirenCounter:Number = 0;
			if (face.type == Face.SHARK_TEETH && tail.type == Tail.SHARK && wings.type == Wings.FEATHERED_LARGE
				&& arms.type == Arms.HARPY) sirenCounter+= 4;
			if (sirenCounter > 0 && hasVagina()) sirenCounter++;
			//if (hasCock() && findFirstCockType(CockTypesEnum.ANEMONE) >= 0) sirenCounter++;
			return sirenCounter;
		}
		//Determine Pig Rating
		...
		public function rhinoScore():Number {
			...
			if (rhinoCounter >= 2 && hasCock()
				&& countCocksOfType(CockTypesEnum.RHINO) > 0) rhinoCounter++;
			return rhinoCounter;
		}
		//Determine Echidna Rating
		...
		//Determine Dragonne Rating
		public function dragonneScore():Number {
			var dragonneCounter:Number = 0;
			if (face.type == Face.CAT) dragonneCounter++;
			if (ears.type == Ears.CAT) dragonneCounter++;
			if (tail.type == Tail.CAT) dragonneCounter++;
			if (tongue.type == Tongue.DRACONIC) dragonneCounter++;
			if (hasDragonWings()) dragonneCounter++;
			if (lowerBody.type == LowerBody.CAT) dragonneCounter++;
			if (hasReptileScales() && dragonneCounter > 0) dragonneCounter++;
			return dragonneCounter;
		}
		//Determine Manticore Rating
		public function manticoreScore():Number {
			var catCounter:Number = 0;
			if (face.type == Face.CAT) catCounter++;
			if (ears.type == Ears.CAT) catCounter++;
			if (tail.type == Tail.SCORPION) catCounter += 2;
			if (lowerBody.type == LowerBody.CAT) catCounter++;
			if (catCounter >= 4) {
				if (horns.type == Horns.DEMON || hasDragonHorns()) catCounter++;
				if (hasLeatheryWings()) catCounter++;
				if (hasLeatheryWings(true)) catCounter++;
			}
			if (hasFur() && catCounter >= 6) catCounter++; //fur only counts if some canine features are present
			return catCounter;
		}
		//Determine Bimbo Rating
		public function bimboScore():Number {
			var bimboCounter:Number = 0;
			if (hasVagina()) {
				bimboCounter += 2;
				if (vaginas[0].vaginalWetness >= VaginaClass.WETNESS_SLICK) bimboCounter++;
			}
		...
		public function hasSpells():Boolean { return spellCount() > 0; }
		...
		public function changeFatigue(mod:Number,type:Number  = 0):void {
			...
			if (type == 2) mod = physicalCost(mod);
			if (type == 3) mod = spellCost(mod);
		...
		public function clothedOrNaked(clothedText:String, nakedText:String = ""):String { return (armorDescript() != "gear" ? clothedText : nakedText); }
		public function clothedOrNakedLower(clothedText:String, nakedText:String = ""):String { return (armorName != "gear" && (armorName != "lethicite armor" && lowerGarmentName == "nothing") && !isTaur() ? clothedText : nakedText); }
		...
		public function shrinkTits(ignore_hyper_happy:Boolean=false):void {
			...
			if (breastRows.length == 1) {
				if (breastRows[0].breastRating > 0) {
					var temp:Number;
					temp = 1;
					breastRows[0].breastRating--; //shrink if bigger than N/A cups
					if (breastRows[0].breastRating >= 1 && rand(2) == 0 && !hasPerk(PerkLib.BigTits)) {
						temp++;
						breastRows[0].breastRating--; //shrink again 50% chance
					}
		...
		public function growTits(amount:Number, rowsGrown:Number, display:Boolean, growthType:Number):void {
			...
			if (growthType == 1) {
				...
				while (rowsGrown > 0) {
					temp = breastRows.length; //temp = counter
					temp2 = 0; //temp2 = smallest tits index
					//Find smallest row
					while (temp > 0) {
						temp--;
						if (breastRows[temp].breastRating < breastRows[temp2].breastRating) temp2 = temp;
					}
					temp3 += amount; //temp3 tracks total amount grown
		...
		public override function minLib():Number {
			...
			if (minLib < (minLust() * 2 / 3))
				minLib = (minLust() * 2 / 3);
			if (jewelryEffectId == JewelryLib.PURITY) minLib -= jewelryEffectMagnitude;
			if (findPerk(PerkLib.PurityBlessing) >= 0) minLib -= 2;
			if (findPerk(PerkLib.HistoryReligious) >= 0) minLib -= 2;
			return minLib;
		}
		...
		public override function minLust():Number {
			...
			if (jewelryEffectId == JewelryLib.MODIFIER_MINIMUM_LUST) {
				...
				if (min > (minCap - jewelryEffectMagnitude) && jewelryEffectMagnitude < 0) minCap += jewelryEffectMagnitude;
			}
		...
		override public function modStats(dstr:Number, dtou:Number, dspe:Number, dinte:Number, dlib:Number, dsens:Number, dlust:Number, dcor:Number, scale:Boolean, max:Boolean):void {
			//Set original values to begin tracking for up/down values if they aren't set yet
			//These are reset when up/down arrows are hidden with 'hideUpDown()'
			...
			if (scale) {
				if (flags[kFLAGS.EASY_MODE_ENABLE_FLAG] == 1 && dlust > 0 && scale) dlust /= 2; //Easy mode cuts lust gains!
				if (findPerk(PerkLib.FutaFaculties) >= 0 || findPerk(PerkLib.BimboBrains) >= 0 || findPerk(PerkLib.BroBrains) >= 0) { //Bimbos learn slower
					if (dinte > 0) dinte /= 2;
					if (dinte < 0) dinte *= 2;
				}
				...
				if (dlust > 0 && scale) dlust *= lustPercent() / 100; //lust resistance
		...
		public function minotaurAddicted():Boolean { return !hasPerk(PerkLib.MinotaurCumResistance) && (hasPerk(PerkLib.MinotaurCumAddict) || flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] >= 1); }
		public function minotaurNeed():Boolean { return !hasPerk(PerkLib.MinotaurCumResistance) && flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] > 1; }
		...
		public function clearStatuses():void {
			...
			if (hasStatusEffect(StatusEffects.Disarmed)) {
				...
				else flags[kFLAGS.BONUS_ITEM_AFTER_COMBAT_ID] = flags[kFLAGS.PLAYER_DISARMED_WEAPON_ID];
			}
		...
		public function hasItem(itype:ItemType, minQuantity:int = 1):Boolean { return itemCount(itype) >= minQuantity; }
		public function itemSlot(idx:int):ItemSlotClass { return itemSlots[idx]; }
		...
		public function killCocks(deadCock:Number):void {
			...
			while (deadCock > 0) {
				...
				while (temp > 0) {
					temp--;
					if (storedCock > cocks.length - 1) storedCock = 0; //if anything is out of bounds set to 0
					if (cocks[temp].cockLength <= cocks[storedCock].cockLength) storedCock = temp; //if temp index is shorter than stored index, store temp to stored index
				}
			...
			if (removed == 1) {
				...
				if (cocks.length == 1) outputText("<b>Your smallest penis disappears, shrinking into your body and leaving you with just one " + cockDescript(0) + ".</b>");
				if (cocks.length > 1)  outputText("<b>Your smallest penis disappears forever, leaving you with just your " + multiCockDescriptLight() + ".</b>");
			}
			if (removed > 1) {
				if (cocks.length == 0) {
				...
				if (cocks.length == 1) outputText("<b>You feel " + num2Text(removed) + " cocks disappear into your groin, leaving you with just your " + cockDescript(0) + ".</b>");
				if (cocks.length > 1)  outputText("<b>You feel " + num2Text(removed) + " cocks disappear into your groin, leaving you with " + multiCockDescriptLight() + ".</b>");
			}
			if (cocks.length == 0) removeStatusEffect(StatusEffects.Infested); //remove infestation if cockless
		...
		public function goIntoHeat(output:Boolean, intensity:int = 1):Boolean {
			if (!hasVagina() || pregnancyIncubation != 0) //no vagina or already pregnant, can't go into heat
				return false;
		...
		The player cannot go into rut if he is a she
		@param	'boolean' indicating if function should output standard text
		@param	'intensity' an integer multiplier that can increase the duration and intensity (defaults to 1)
		@return  true if successful, false if not*/
		public function goIntoRut(output:Boolean, intensity:int = 1):Boolean {
			if (!hasCock()) //no cocks, can't go into rut
				return false;
			...
			//Go in rut. Rut v1 is cum production, v2 is bonus libido, v3 is hours till it's gone
			else {
		...
		public function setFurColor(colorArray:Array, underBodyProps:Object = null, doCopySkin:Boolean = false, restoreUnderBody:Boolean = true):void { _setSkinFurColor("fur", colorArray, underBodyProps, doCopySkin, restoreUnderBody); }
		public function setSkinTone(colorArray:Array, underBodyProps:Object = null, doCopySkin:Boolean = false, restoreUnderBody:Boolean = true):void { _setSkinFurColor("skin", colorArray, underBodyProps, doCopySkin, restoreUnderBody); }
		...
		public function HPChange(changeNum:Number, display:Boolean):Number {
			...
			if (changeNum > 0) {
				if (findPerk(PerkLib.HistoryHealer) >= 0) changeNum *= 1.2; //increase by 20%!
				if (armor.name === "skimpy nurse's outfit") changeNum *= 1.1; //increase by 10%!
		...
		public function HPChangeNotify(changeNum:Number):void {
			if (changeNum === 0)
				if (HP >= maxHP()) outputText("You're as healthy as you can be.\n");
			else if (changeNum > 0) {
				if (HP >= maxHP()) outputText("Your HP maxes out at " + maxHP() + ".\n");
				else outputText("You gain <b><font color=\"#008000\">" + int(changeNum) + "</font></b> HP.\n");
			}
			else {
				if (HP <= 0) outputText("You take <b><font color=\"#800000\">" + int(changeNum*-1) + "</font></b> damage, dropping your HP to 0.\n");
				else outputText("You take <b><font color=\"#800000\">" + int(changeNum*-1) + "</font></b> damage.\n");
			}
		}
	}
}